#!/usr/bin/env bash
# Stop and remove OLD DevOps demo local resources (Docker)
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo "==> Stopping Jenkins..."
(cd "${ROOT}/jenkins" && docker compose -f docker-compose.jenkins.yml down -v 2>/dev/null) || true
...
echo ""
echo "AWS stacks must be deleted in AWS Console (see Note/ecs-start.html Phase 0/6)."
